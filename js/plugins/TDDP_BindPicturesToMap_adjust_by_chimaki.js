...
Imported.MapData = {//mapId / pictireName
// "地圖id":{
//         圖層:[圖片名稱, 原始座標(通常為0), x, y,寬度倍率(0~100), 高度倍率, 透明度(0~255), 混和模式(0~4)),插件的圖層參數]
//     };                                             
    "19" :{ 
            "1":['001_CatMap',0,0,0,100,100,255,0,'below_tilemap'],//記住每個圖片要用逗號隔開
            "2":['001_CatMap_影',0,0,0,100,100,255,0,'below_tilemap'],
            "3":['001_CatMap光',0,0,0,100,100,255,0,'below_tilemap'],
        },
...
(function() {
    ...
    //粽子修改
    var Parameters = PluginManager.parameters('TDDP_BindPicturesToMap');
    var switchId = Number(Parameters['ParallaxSwitchId'])|| 15;
    ...
    Scene_Map.prototype.setBindMode = function (){
        
        $gameMap._interpreter.checkBindPictureToMap();        
    }
    Game_Interpreter.prototype.checkBindPictureToMap = function(){
    $gameScreen.clearPictures();
    var nowMapId = $gameMap._mapId;
    for (var index in Imported.MapData){
            if (index == $gameMap._mapId){
                for (var layer in Imported.MapData[index]){
                    var pictureId = layer;
                    var name = Imported.MapData[index][layer][0];
                    var origin = Imported.MapData[index][layer][1]|| 0;
                    var x = Imported.MapData[index][layer][2]||0;
                    var y = Imported.MapData[index][layer][3]||0;
                    var scaleX = Imported.MapData[index][layer][4]||100;
                    var scaleY = Imported.MapData[index][layer][5]||100;
                    var opacity = Imported.MapData[index][layer][6]||255;
                    var blendMode = Imported.MapData[index][layer][7]||0;
                    var bindMode = Imported.MapData[index][layer][8]||'below_tilemap';
                    $gameScreen.showPicture(
                        pictureId, name, origin, x, y,scaleX, scaleY, opacity,blendMode );                                             
                    $gameMap._interpreter.bindPictureToMap(pictureId,true,bindMode);
                }                
            }
            else {
                var name = $gameMap._parallaxName;
                $gameScreen.showPicture(1, name, 0, 0, 0, 100, 100, 255, 0);                                             
                $gameMap._interpreter.bindPictureToMap(1,true,'below_tilemap');
            }            
        }
    };
    ...
    Spriteset_Map.prototype.update = function() {
        // _Spriteset_Map_update.call(this); TOOP原本是這行
        //粽子修改========================================
        //覆寫掉原本的,在地途中使用開關決定是否開啟遠景
            Spriteset_Base.prototype.update.call(this);                        
            this.updateTileset();
            if ($gameSwitches.value(switchId)){this.updateParallax()};
            this.updateTilemap();
            this.updateShadow();
            this.updateWeather();
            this.updatePictures();            
